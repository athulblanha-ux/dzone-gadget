...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Total Revenue', value: `₹${data?.summary?.totalRevenue?.toLocaleString() || 0}`, icon: FiDollarSign, color: 'text-green-500', bg: 'bg-green-100 dark:bg-green-900/20' },
    { label: 'Total Orders', value: data?.summary?.totalOrders || 0, icon: FiShoppingBag, color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/20' },
    { label: 'Total Customers', value: data?.summary?.totalUsers || 0, icon: FiUsers, color: 'text-purple-500', bg: 'bg-purple-100 dark:bg-purple-900/20' },
    { label: 'Total Products', value: data?.summary?.totalProducts || 0, icon: FiBox, color: 'text-orange-500', bg: 'bg-orange-100 dark:bg-orange-900/20' },
  ];
            ...
            {data?.topProducts?.length ? data.topProducts.map((p) => (
              <div key={p._id} className="flex items-center gap-4 border-b dark:border-dark-border pb-4 last:border-0 last:pb-0">
                <div className="w-12 h-12 bg-gray-100 dark:bg-dark-bg rounded-lg overflow-hidden">
                  {p.image ? <img src={p.image} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><img src="/logo.png" className="w-6 h-6 object-contain opacity-50" alt="logo" /></div>}
                </div>
                ...
                <div className="font-bold text-primary-600 text-sm">₹{p.price?.toLocaleString()}</div>
              </div>
